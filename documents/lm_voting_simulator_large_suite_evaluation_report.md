...
This report summarizes the second large-scale E01-E06 experiment suite and uses the first small-scale E01-E06 suite as a secondary reference. The old overall report, `documents/lm_voting_simulator_evaluation_report.md`, is retained and not overwritten.

## 1. Executive Summary

The conclusions from the second large-scale experiment suite are clearer than those from the first round.

At the engineering level, the system is now relatively stable. E01-large through E06-large all completed, and all quality gates passed; 100% of LLM outputs followed the hard-choice contract, with no invalid choice, forbidden choice, legacy probability schema, or transport error. Concurrent execution was stable and the GPU had no OOM. E06-large explicitly made no Ollama calls and only reused prior artifacts for local table analysis.

At the scientific level, the current `qwen3.5:2b` cannot be treated as a reliable voter simulator. It can reliably emit correctly formatted hard choices and can recover individual voting direction when party/ideology is explicit; however, it does not stably perform fine-grained persona simulation from rich survey memory. Its most reliable information source is party/ideology. After candidate names, year, state names, candidate context, or strict memory context are added, the model often shows strong Republican skew, especially harming Democratic-leaning subgroups or states such as Lean Democrat, Independent, Black respondents, and CO/VA/MN.

The most important cross-experiment conclusions:

- The hard-choice contract is the correct engineering fix and fully avoids the old probability JSON problems of normalization and fabricated probabilities.
- The large runs found no fatal issues in parsing, schema, concurrency, or caching.
- `party/ideology` is the most effective non-oracle LLM condition for the current small model.
- `strict memory` itself contains some information, but under the current prompt design it does not stably exceed party/ideology.
- `candidate/context` framing significantly amplifies Republican collapse.
- After E02-large adds MN/VA/CO, LLM strict can no longer hide behind "all seven swing states are Republican winners"; it predicts MN/VA/CO as Republican too.
- E03-large confirms that candidate order is not a small-sample accident, and `candidate_order_reversed` remains the most sensitive prompt variant.
- E04-large shows that the model is close to balanced with anonymous candidates, but once party/candidate/year context appears, output strongly shifts toward Republican.
- E05-large's full ladder shows that party/ideology brings the largest gain, candidate context brings the largest harm, oracle can recover the correct answer, and placebo memory is basically ineffective.
- E06-large localizes the overall problem to concrete subgroups: errors are most prominent for Independent, Lean Democrat, Black respondents, and Democratic-winning states.

Overall assessment: this evaluation framework can now continue to be used as a systematic experiment platform; however, the current `qwen3.5:2b` should only be treated as an MVP small-model baseline, not as the final simulator model.

## 2. Response Contract and Interpretation

All CES turnout-vote LLM tasks in this round use the three-class hard-choice contract:
...
- `turnout_probability = 1` iff `choice` is `democrat` or `republican`
- `turnout_probability = 0` iff `choice` is `not_vote`
- `vote_prob_democrat = 1` iff `choice == "democrat"`
- `vote_prob_republican = 1` iff `choice == "republican"`
- `vote_prob_other = 0`
- `vote_prob_undecided = 0`

These probability columns are not LLM subjective probabilities. They are only one-hot derived values from the hard choice. All calibration / Brier / entropy diagnostics in the report are interpreted under this principle and must not be read as "whether the model probabilities are calibrated."
...
Total wall-clock for E01-E06-large was about 93.95 minutes, with no LLM calls in E06. All LLM large runs stayed within 30 minutes.
...
The large runs were clearly faster than the initial estimate. GPU peak memory was about 5.0-5.2 GiB, with minimum free memory about 2.7-2.9 GiB, indicating that 3-4 workers is a stable high-throughput configuration.

### 4.3 Quality gates

All large formal runs passed the parsing quality gates:
...
This means this round's conclusions are mainly about model behavior, not artifacts caused by format failure, concurrency failure, or parsing failure.

## 5. E01-Large: Individual Persona Fidelity

### 5.1 Goal and setup

E01-large checks whether LLM agents can reproduce each CES respondent's own 2024 turnout/vote hard choice at the individual level.
...
The ranking in E01-large is clear:

1. Statistical models `sklearn_logit_pre_only` / `poll_informed` are strongest.
2. Among LLM conditions, `party/ideology` is strongest.
3. `strict memory` is below `party/ideology`.
4. demographics-only is weak.

Compared with E01-small:

- In E01-small, `party/ideology` and `strict memory` were almost tied, both with vote accuracy around 0.977.
- In E01-large, they separate: `party/ideology=0.914`, `strict memory=0.871`.

This shows that strict memory looked "not bad" in the small sample, but after expanding to 10 states and 1000 agents, it has no stable gain. The current small model mainly uses party/ideology, not rich survey memory.

## 6. E02-Large: Aggregate Election Accuracy

### 6.1 Goal and setup

E02-large checks whether state-level aggregation can reproduce 2024 presidential election Democratic two-party share, margin, and winner.

Design:

- States: PA, MI, WI, GA, AZ, NV, NC, MN, VA, CO
- The original 7 states were all 2024 Republican winners
- Added MN/VA/CO were 2024 Democratic winners
...
E02-small achieved LLM winner accuracy of 1.0 on seven all-Republican-winner states, but margin MAE was very poor. After E02-large adds MN/VA/CO, the problem is exposed: LLM strict also predicts MN, VA, and CO as Republican, winner accuracy falls to 0.7, and margin MAE remains as high as 0.314.

Conclusion: LLM strict is not a reliable aggregate predictor. It has strong Republican skew and cannot stably recover Democratic-winning states.

## 7. E03-Large: Prompt Robustness

### 7.1 Goal and setup

E03-large checks whether prompt wording changes hard-choice outputs under the same persona and the same strict memory.
